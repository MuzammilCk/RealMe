...
import { ACCENT, BRASS } from '../colors';
  ...
  return (
    <group position={[2.85, 0.1, 0.15]} rotation={[0, -0.5, 0]}>
      ...
      >
        ...
        <meshStandardMaterial color={ACCENT.camera} roughness={0.5} metalness={0.3} />
      </mesh>
      <mesh position={[0, 0, 0.15]} rotation={[Math.PI / 2, 0, 0]}>
        <cylinderGeometry args={[0.075, 0.09, 0.14, 20]} />
        <meshStandardMaterial color={ACCENT.lens} roughness={0.3} metalness={0.5} />
      </mesh>
      <mesh position={[0.12, 0.13, 0]} rotation={[Math.PI / 2, 0, 0]}>
        <cylinderGeometry args={[0.03, 0.03, 0.04, 12]} />
        <meshStandardMaterial color={BRASS[700]} roughness={0.4} metalness={0.6} />
      </mesh>
    </group>
  );
}
